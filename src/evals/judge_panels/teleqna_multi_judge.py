...
# ── Judge prompt ──────────────────────────────────────────────────────────────
GRADER_TEMPLATE = """
You are a Telecommunications Domain Expert (3GPP, 5G, LTE, Open RAN). Judge whether the [Student Answer] is technically correct using your expert domain knowledge. Use [correct_answer] as a reference guide.
...
extracted_final_answer: Copy the full [Student Answer] text verbatim. Only write None if [Student Answer] is literally empty — do NOT write None because the answer seems wrong or irrelevant.
reasoning: One sentence on why [Student Answer] is correct or incorrect based on your expert 3GPP domain knowledge. Reference [correct_answer] only as a guide. Do not solve the problem.
GRADE: C if the [Student Answer] is factually correct per your expert 3GPP domain knowledge and consistent with [correct_answer]. Make sure your GRADE aligns with your reasoning. If not factually correct,GRADE: I.
reasoning_quality: Integer 0-10 rating how factually correct and well-supported the student's reasoning in [Student Explanation] is, judged primarily by your expert 3GPP domain knowledge, with [correct_answer] as a reference guide.
...
# Primary: labelled match, tolerant of common judge output variants.
...
def _parse_reasoning_quality(judge_output: str) -> tuple[float, int | None, bool]:
    # Primary parse: strict labeled line.
    ...
    # Fallback 1: labeled-but-not-line-anchored form.
    ...
    # Fallback 2: final standalone integer line.
...
def _sanitize_model_name(model_name: str) -> str:
    # Extract model basename (e.g. gemini-3.5-flash)
    name = model_name.split("/")[-1]
    # Strip common suffixes/extensions
...
def _infer_grade_from_reasoning(judge_output: str) -> str | None:
    """Infer a likely grade ('C' or 'I') from the judge's reasoning text.

    Returns 'C' if the reasoning clearly indicates semantic equivalence,
    'I' if it clearly indicates non-equivalence, or None if ambiguous.
    """
...
def teleqna_scorer(
    ...
    # Dynamically build the metrics dictionary for all active judges
    metric_keys = {
    ...
    def _scorer():
        async def score(state, target):
            ...
            for judge_model in judge_models:
                ...
                # Post-parse consistency: infer grade from reasoning text and
                # correct obvious contradictions between textual reasoning and
                # the parsed GRADE field.
            ...
            # Add the overall average metrics across all judges
